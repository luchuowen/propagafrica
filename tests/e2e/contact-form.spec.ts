...
test.describe('quotation form markup and progressive enhancement', () => {
  test('every field has a label, a stable id and an aria-describedby error target', async ({
    ...
    // "Stage" is a fieldset of checkboxes, not a single input with an id.
  ...
  test('prefills fields from the query string', async ({ page }) => {
    await page.goto('/contact?name=Jane+Wanjiru&country=Kenya&crop=Rose&stage=Graft&stage=Root');
    await expect(page.locator('#name')).toHaveValue('Jane Wanjiru');
    await expect(page.locator('#country')).toHaveValue('Kenya');
    await expect(page.locator('#crop')).toHaveValue('Rose');
    await expect(page.locator('#stage-1')).toBeChecked(); // Graft
    await expect(page.locator('#stage-2')).toBeChecked(); // Root
    await expect(page.locator('#stage-0')).not.toBeChecked(); // Prepare
  });
  ...
  test('shows inline errors and does not navigate when required fields are empty', async ({
    ...
    await expect(page.locator('#stage-error')).toHaveText('Select at least one stage.');
